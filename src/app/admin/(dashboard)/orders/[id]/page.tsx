    ...
    <section className="space-y-5">
      ...
      <div>
        ...
        <p className="mt-1 pl-6 text-xs text-gray-500">
          Review order information, products and status.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
        ...
        <SummaryItem
          label="Customer"
          value={order.customer.fullName || "Customer"}
          secondary={order.customer.email}
        />

        <SummaryItem label="Products" value={String(productCount)} />

        <SummaryItem label="Sub Total" value={money(order.subtotal)} />

        <SummaryItem label="Tax" value={money(order.tax)} />

        <SummaryItem label="Total" value={money(order.total)} strong />
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm shadow-gray-100/50">
              ...
              <tbody>
                {order.items.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="max-w-65 truncate text-xs font-medium text-gray-900">
                            {item.productName}
                          </p>
                    ...
                    <td className="px-4 py-3">
                      <CurrentStock stock={item.currentStock} />
                    </td>
          ...
          <div className="border-t border-gray-100 bg-gray-50/40 px-4 py-3">
            <div className="ml-auto max-w-65 space-y-2">
...
function SummaryItem({
  ...
  return (
    <div className="min-w-0 rounded-xl border border-gray-200 bg-white px-3 py-3 shadow-sm shadow-gray-100/50">
      ...
      {secondary ? (
        <p className="mt-0.5 truncate text-[10px] text-gray-400">{secondary}</p>
      ) : null}
    </div>
  );
}
...
function CurrentStock({ stock }: { stock: number | null }) {
  if (stock === null) {
