...
export default function ResetPasswordForm({
  ...
  const [form, setForm] =
    useState<ResetPasswordFormState>(initialForm);
      ...
      if (!response.ok || !data.success) {
        ...
        if (data.errors) {
          for (const field of Object.keys(
            data.errors
          ) as Array<keyof ResetPasswordInput>) {
    ...
    <>
      ...
      >
        ...
        <p className="text-center text-sm text-[#6c757d]">
          ...
          <Link
            href="/login"
            className="font-medium text-[#087ff5] transition-colors hover:text-[#066ed6] hover:underline focus-visible:rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#087ff5] focus-visible:ring-offset-2"
          >
            Login
          </Link>
        </p>
