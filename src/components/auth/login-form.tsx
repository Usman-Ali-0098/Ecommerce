...
export default function LoginForm() {
  ...
  async function handleGoogleSignIn() {
    try {
       ...
       * customer-only in auth.ts.
       *
       * Auth.js handles the redirect
       * through Google and returns
       * the customer to "/".
       */
      await signIn("google", {
        callbackUrl: "/",
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
       ...
       *
       * redirect:false lets us inspect
       * the resulting session and route
       * according to USER / ADMIN.
       */
      ...
      if (!session?.user?.role) {
         ...
         * but a usable application
         * session was not produced,
         * remove it.
         */
        await signOut({
          redirect: false,
        ...
        showAlert("Unable to create your session.", {
          variant: "error",
        });
       ...
       */
      const destination =
        session.user.role === "ADMIN" ? "/admin/products" : "/";

      /*
       * replace() is intentional.
       *
       * We do not want /login to remain
